  ...
  list(String tag, List<Map<String, dynamic>> array) {
    ...
    for (int i = 0; i < array.length; ++i) {
      ...
      if (array[i]['data'] is String) {
        ...
        if (array[i]['data'] is StropheBuilder)
          stanza = array[i]['data'].tree();
...
*/
  ...
  String service;
  String jid;
  Map<String, List<StropheHandler>> handler = {};
   ...
   */
  storeHandler(String node, StropheHandler handler) {
  ...
  removeHandler(String node) {
    List<StropheHandler> toberemoved = this.handler[node];
    this.handler[node] = [];
  ...
  discoverNodes(
    ...
    service = service != null && service.isNotEmpty ? service : this.service;
    StropheBuilder iq =
        Strophe.$iq({'from': this.jid, 'to': service, 'type': 'get'})
  ...
  String items(String node, [Function success, Function error, int timeout]) {
    //ask for all items
    StropheBuilder iq =
        Strophe.$iq({'from': this.jid, 'to': this.service, 'type': 'get'})
            .c('pubsub', {'xmlns': Strophe.NS['PUBSUB']}).c(
                'items', {'node': node});
