...
class DiscoPlugin extends PluginClass {
  ...
  _onDiscoInfo(XmlElement stanza) {
    ...
    StropheBuilder iqresult = this._buildIQResult(stanza, attrs);
    for (int i = 0; i < this._identities.length; i++) {
      attrs = {
  ...
  Future<bool> _onDiscoItems(XmlElement stanza) async {
    ...
    StropheBuilder iqresult = this._buildIQResult(stanza, queryAttrs);
